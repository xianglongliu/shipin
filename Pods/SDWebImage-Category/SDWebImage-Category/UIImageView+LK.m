...
- (void)setImageURL:(id)imageURL
{
    ...
    imageURL = [self lk_URLWithImageURL:imageURL];
    if (self.sd_imageURL && self.image && self.image.duration == 0 && self.status == LKImageViewStatusLoaded && imageURL)
    {
        if([[self.sd_imageURL absoluteString] isEqualToString:[imageURL absoluteString]])
        {
            ///相同的图片URL 就不在设置了
            return;
        }
...
{
    if(self.sd_imageURL == nil && [self lk_hasLoadedTapEvent])
    {
        return NO;
    }
    ...
    if (self.sd_imageURL && self.onTouchTapBlock == nil)
    {
        return NO;
    }
    return [super pointInside:point withEvent:event];
